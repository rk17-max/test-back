...
// MongoDB Connection
const mongoURI = 'mongodb://localhost:27017/myapp'; // Replace with your actual MongoDB URI
...
app.get('/files', async (req, res) => {
  try {
      ...
      fs.readdir('uploads', async (err, fileNames) => {
          ...
          console.log('File Names from Uploads Directory:', fileNames); // Log file names

          // Fetch file metadata from the database
          const files = await File.find({ originalName: { $in: fileNames } });
          console.log('Fetched Files from Database:', files); // Log fetched files
          
          // Check if there are files in the database and if names match
          if (files.length === 0) {
              console.warn('No matching files found in the database for:', fileNames);
          }
          ...
          console.log('File Details:', fileDetails); // Log the final details
...
app.get('/filesdelete/:id', verifyUser, async (req, res) => {
  try {
      ...
      fs.unlink(filePath, async (err) => {
          ...
          // Optionally, you can also remove the file ID from the user's uploadedFiles array
